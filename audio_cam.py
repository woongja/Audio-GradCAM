...
class AudioGradCAM:
    def __init__(
        ...
        target_class: int = 1, # target_class 추가
    ...
    def forward(self, input_tensor: torch.Tensor,
        ...
        # self.visualize_cam_1(cam, normalized_cam)
    ...
    def visualize_cam(self, cam, normalized_cam):
        ...
        # Plot Grad-CAM heatmap
        vmax = np.max(np.abs(normalized_grad_cam_values))
        norm = TwoSlopeNorm(vmin=-vmax, vcenter=0, vmax=vmax)
        im = ax2.imshow(normalized_grad_cam_values[np.newaxis, :], cmap='RdBu_r', norm=norm, 
                        aspect='auto', extent=[0, duration, 0, 1])
        ax2.set_title('Grad-CAM Heatmap')
        ax2.set_xlabel('Time (s)')
        ...
        ax3.bar(frame_times, normalized_cam.squeeze(), width=(duration/num_frames)*1.005, align='center', color=colors, alpha=0.7)
        ax3.set_title('LA_E_1837629.flac')
        ax3.set_xlabel('Time (s)')
        ax3.set_ylabel('Score')
        ax3.set_ylim(-1, 1)
        ax3.axhline(y=0, color='k', linestyle='--', linewidth=0.5)  # 0 기준선 추가
        ...
        audio_widget = Audio(self.waveform, rate=self.sr)
        display(audio_widget)
    def visualize_cam_1(self, cam, normalized_cam):
        if self.waveform is None or self.sr is None:
            raise ValueError("Waveform and sample rate are not set. Please run forward() first.")
        ...
        # Create the figure with 2 subplots (waveform, bar chart)
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 6), gridspec_kw={'height_ratios': [1, 1]})

         # Plot the waveform (Amplitude as is, no normalization)
        time_axis = np.linspace(0, duration, len(self.waveform))
        ax1.plot(time_axis, self.waveform, color='black')  # 진폭 그대로 그리기
        ax1.set_title('Waveform')
        ax1.set_xlabel('Time (s)')
        ax1.set_ylabel('Amplitude')
        ...
        # Plot normalized Grad-CAM scores as a bar chart with colors based on original values
        colors = ['blue' if v < 0 else 'red' for v in cam.squeeze()]
        ax2.bar(frame_times, normalized_cam.squeeze(), width=(duration/num_frames)*1.005, align='center', color=colors, alpha=0.7)
        # ax2.set_title('LA_E_1837629.flac')
        ax2.set_xlabel('Time (s)')
        ax2.set_ylabel('Score')
        ax2.set_ylim(-1, 1)
        ax2.axhline(y=0, color='k', linestyle='--', linewidth=0.5)  # 0 기준선 추가
...
#     display(audio_widget)
def visualize_cam(normalized_cam, audio_path):
    # Load the audio file
    waveform, sr = sf.read(audio_path)
    normalized_grad_cam_values = normalized_cam.squeeze()
    # Calculate frame times
    num_frames = len(normalized_grad_cam_values)
    duration = len(waveform) / sr
    frame_times = np.linspace(0, duration, num_frames)
    ...
    # Plot the waveform with original amplitude
    # librosa.display.waveshow(waveform, sr=sr, ax=ax1, color='black')  # Change color to black
    ax1.plot(np.arange(len(waveform)) / sr, waveform, color='black')  # Change color to black
    ax1.set_title('CON_E_0033629.wav')
    ax1.set_ylabel('Amplitude')

    # Determine colors based on original Grad-CAM values
    colors = ['blue' if v < 0 else 'red' for v in normalized_grad_cam_values.squeeze()]

    # Plot normalized Grad-CAM scores as a bar chart with color based on original values
    ax2.bar(frame_times, normalized_grad_cam_values.squeeze(), width=(duration/num_frames)*1.005, align='center', color=colors, alpha=0.7)
    ax2.set_title('Normalized Grad-CAM Scores')
    ax2.set_xlabel('Time (s)')
    ax2.set_ylabel('Score')
    ax2.set_ylim(-1, 1)  # Set y-axis limit to -1 and 1
    ax2.axhline(y=0, color='k', linestyle='--', linewidth=0.5)  # Add horizontal line at y=0
